...
const Field = ({
  id, label, value, onChange, onClearErr, errKey, errors,
  type = "text", placeholder = "",
  isDark: _isDark, inputBg, border, txtPrim, txtLabel,
}: FieldProps) => {
  ...
  return (
    <div style={{ width: "100%" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        {hasErr && (
          <span
            role="alert"
            aria-live="polite"
            style={{ fontSize: 11, color: "#EC5757" }}
          >
            {errors[errKey]}
          </span>
        )}
      ...
      <input
        ...
        style={inputStyle}
        onFocus={(e) => { e.target.style.borderColor = hasErr ? "#EC5757" : "#7C5DFA"; }}
        onBlur={(e)  => { e.target.style.borderColor = hasErr ? "#EC5757" : border; }}
      />
    </div>
  );
};
...
export const InvoiceForm = () => {
  ...
  // Pre-populate or reset when form opens
  useEffect(() => {
  ...
  const addItem = () =>
    setItems((p) => [...p, { id: Date.now().toString(), name: "", qty: 1, price: 0 }]);

  const removeItem = (id: string) =>
    setItems((p) => p.filter((i) => i.id !== id));

  const updateItemField = (id: string, field: keyof Item, value: string | number) =>
    setItems((p) => p.map((i) => (i.id === id ? { ...i, [field]: value } : i)));

  // ── Theme values ──────────────────────────────────────────────────────────
  const pageBg   = isDark ? "var(--bg-page)"   : "#fff";
  const cardBg   = isDark ? "var(--bg-card)"   : "#fff";
  const txtPrim  = isDark ? "#FFFFFF"           : "#0C0E16";
  const txtLabel = isDark ? "#888EB0"           : "#7E88C3";
  const border   = isDark ? "#252945"           : "#DFE3FA";
  const inputBg  = isDark ? "#252945"           : "#FFFFFF";

  const itemInputStyle = (hasErr: boolean): CSSProperties => ({
    width: "100%",
    background: inputBg,
    border: `1px solid ${hasErr ? "#EC5757" : border}`,
    borderRadius: 4,
  ...
  const errTxt: CSSProperties = {
    fontSize: 11, color: "#EC5757", marginTop: 4, display: "block",
  };
  ...
  return (
    <>
      <style>{`
        ...
        @media (min-width: 768px) { .inv-drawer { margin-left: 72px; } }
        ...
        .item-grid {
          ...
          gap: 12px;
          align-items: start;
          grid-template-columns: 1fr 52px 88px 56px 20px;
        }
        .item-hdr {
          display: grid;
          gap: 12px;
        ...
        @media (max-width: 400px) {
          .item-grid { grid-template-columns: 1fr 40px 70px 46px 20px; gap: 6px; }
        ...
        .section-title {
          font-size: 12px; font-weight: 700; color: #7C5DFA;
      ...
      >
        <div className="inv-drawer" style={{ background: pageBg }}>

          {/* ── Scrollable body ─────────────────────────── */}
          <div className="inv-scroll">
            <h2 style={{
              fontFamily: "Syne, sans-serif", fontWeight: 800,
              fontSize: 24, color: txtPrim, margin: "0 0 32px",
            }}>
              {isEditing
            ...
            <fieldset style={{ border: "none", padding: 0, margin: "0 0 40px" }}>
              ...
              <div style={{ marginBottom: 16 }}>
                <Field id="fromStreet" label="Street Address"
                  value={fromStreet} onChange={setFromStreet}
                  onClearErr={clearErr} errKey="fromStreet" errors={errors}
                  isDark={isDark} inputBg={inputBg} border={border} txtPrim={txtPrim} txtLabel={txtLabel} />
              </div>
              <div className="g3">
                <Field id="fromCity" label="City"
                  value={fromCity} onChange={setFromCity}
                  onClearErr={clearErr} errKey="fromCity" errors={errors}
                  isDark={isDark} inputBg={inputBg} border={border} txtPrim={txtPrim} txtLabel={txtLabel} />
                <Field id="fromPost" label="Post Code"
                  value={fromPost} onChange={setFromPost}
                  onClearErr={clearErr} errKey="fromPost" errors={errors}
                  isDark={isDark} inputBg={inputBg} border={border} txtPrim={txtPrim} txtLabel={txtLabel} />
                <Field id="fromCountry" label="Country"
                  value={fromCountry} onChange={setFromCountry}
                  onClearErr={clearErr} errKey="fromCountry" errors={errors}
                  isDark={isDark} inputBg={inputBg} border={border} txtPrim={txtPrim} txtLabel={txtLabel} />
              </div>
            </fieldset>
            ...
            <fieldset style={{ border: "none", padding: 0, margin: "0 0 40px" }}>
              ...
              <div style={{ marginBottom: 16 }}>
                <Field id="clientName" label="Client's Name"
                  value={clientName} onChange={setClientName}
                  onClearErr={clearErr} errKey="clientName" errors={errors}
                  isDark={isDark} inputBg={inputBg} border={border} txtPrim={txtPrim} txtLabel={txtLabel} />
              </div>
              <div style={{ marginBottom: 16 }}>
                <Field id="clientEmail" label="Client's Email"
                  value={clientEmail} onChange={setClientEmail}
                  type="email" placeholder="e.g. email@example.com"
                  onClearErr={clearErr} errKey="clientEmail" errors={errors}
                  isDark={isDark} inputBg={inputBg} border={border} txtPrim={txtPrim} txtLabel={txtLabel} />
              </div>
              <div style={{ marginBottom: 16 }}>
                <Field id="toStreet" label="Street Address"
                  value={toStreet} onChange={setToStreet}
                  onClearErr={clearErr} errKey="toStreet" errors={errors}
                  isDark={isDark} inputBg={inputBg} border={border} txtPrim={txtPrim} txtLabel={txtLabel} />
              </div>
              <div className="g3" style={{ marginBottom: 16 }}>
                <Field id="toCity" label="City"
                  value={toCity} onChange={setToCity}
                  onClearErr={clearErr} errKey="toCity" errors={errors}
                  isDark={isDark} inputBg={inputBg} border={border} txtPrim={txtPrim} txtLabel={txtLabel} />
                <Field id="toPost" label="Post Code"
                  value={toPost} onChange={setToPost}
                  onClearErr={clearErr} errKey="toPost" errors={errors}
                  isDark={isDark} inputBg={inputBg} border={border} txtPrim={txtPrim} txtLabel={txtLabel} />
                <Field id="toCountry" label="Country"
                  value={toCountry} onChange={setToCountry}
                  onClearErr={clearErr} errKey="toCountry" errors={errors}
                  isDark={isDark} inputBg={inputBg} border={border} txtPrim={txtPrim} txtLabel={txtLabel} />
              </div>
              <div className="g2" style={{ marginBottom: 16 }}>
                <div>
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    <label htmlFor="invDate" style={{
                      fontSize: 12, fontWeight: 500,
                      color: errors.invoiceDate ? "#EC5757" : txtLabel,
                    }}>
                      Invoice Date
                    </label>
                    {errors.invoiceDate && (
                      <span role="alert" style={{ fontSize: 11, color: "#EC5757" }}>
                        {errors.invoiceDate}
                      </span>
                    )}
                  </div>
                  <input
                    ...
                    style={{
                      ...itemInputStyle(!!errors.invoiceDate),
                      padding: "12px 16px", marginTop: 6,
                    }}
                  />
                </div>
                <div>
                  <label htmlFor="payTerms" style={{ fontSize: 12, fontWeight: 500, color: txtLabel }}>
                    Payment Terms
                  </label>
                  <select
                    id="payTerms"
              ...
              <Field id="description" label="Project Description"
                ...
                onClearErr={clearErr} errKey="description" errors={errors}
                isDark={isDark} inputBg={inputBg} border={border} txtPrim={txtPrim} txtLabel={txtLabel} />
            </fieldset>

            {/* Item List */}
            <section aria-label="Invoice items" style={{ marginBottom: 24 }}>
              <p style={{ fontSize: 18, fontWeight: 700, color: "#777F98", margin: "0 0 20px" }}>
                Item List
              </p>
              ...
              {items.map((item) => (
                <div key={item.id} style={{ marginBottom: 16 }}>
                  <div className="item-grid">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          clearErr(`item-name-${item.id}`);
                        }}
                        style={itemInputStyle(!!errors[`item-name-${item.id}`])}
                      />
                    </div>
                    {/* Qty */}
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          clearErr(`item-qty-${item.id}`);
                        }}
                        style={{ ...itemInputStyle(!!errors[`item-qty-${item.id}`]), textAlign: "center" }}
                      />
                    </div>
                    {/* Price */}
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          clearErr(`item-price-${item.id}`);
                        }}
                        style={itemInputStyle(!!errors[`item-price-${item.id}`])}
                      />
                    </div>
                    {/* Total */}
                    <span style={{
                      fontSize: 13, fontWeight: 700, color: "#888EB0",
                    ...
                    <button
                      ...
                      style={{
                        ...
                        color: "#888EB0", paddingTop: 12, transition: "color 0.15s",
                      }}
                  ...
                  {(errors[`item-name-${item.id}`] || errors[`item-qty-${item.id}`] || errors[`item-price-${item.id}`]) && (
                    <div style={{ display: "flex", gap: 12, marginTop: 4, flexWrap: "wrap" }} role="alert">
              ...
              {itemsError && (
                <p style={{ color: "#EC5757", fontSize: 12, marginBottom: 8, fontWeight: 600 }} role="alert">
                  {itemsError}
                </p>
              )}
          ...
          <div
            ...
            style={{ background: cardBg, boxShadow: "0 -4px 12px rgba(0,0,0,0.10)" }}
          >
            {isEditing ? (
              <>
